...
switch(this.state){   
    ...
    case this.STATE_PLAY:
           ...
           //PLAYER LOOP
            this.playerGroup.forEachAlive(function(player){
                ...
                if ( player.score>100 && this.MODE == this.ULTRA_SIMPLE_MODE ) { this.onDeath(player); console.log("hhello?"); }
    ...
    case this.STATE_GAMEOVER:    
                this.CURRENT_RUN++;
                if(this.CURRENT_RUN == this.NUMBER_OF_PLAYERS*0.05){
                    this.MODE = this.SIMPLE_MODE;
                    console.log(this.mode);

                }
                if(this.CURRENT_RUN == this.NUMBER_OF_PLAYERS*0.1)
