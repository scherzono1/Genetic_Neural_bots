...
App.Main = function(game){
    ...
    this.NUMBER_OF_PLAYERS = 200;
    this.NUMBER_OF_GROUND = 30;
    this.STATE_INIT = 1;
    this.STATE_START = 2;
    this.STATE_PLAY = 3;
    this.STATE_GAMEOVER = 4;
    this.BARRIER_DISTANCE = 250;
...
App.Main.prototype = {
    ...
    //GA
   this.GA = new GeneticAlgorithm(this.NUMBER_OF_PLAYERS, this.NUMBER_OF_PLAYERS*0.1); //(population, top who evolve)
    //GROUND
...
switch(this.state){   
    ...
    case this.STATE_START:        
            ...
            this.groundGroup.forEach(function(ground){
                    if(i0!=0 && this.MODE == this.SIMPLE_MODE) ground.restart(this.game, this.groundGroup.getAt(ground.index-1).world.x + this.distanceVect[i0], this.heightVect[i0]);
                    else if (i0!=0 && this.MODE == this.RANDOM_MODE) ground.restart(this.game, this.groundGroup.getAt(ground.index-1).world.x 
    ...
    case this.STATE_PLAY:
            ...
            if (this.firstGround.world.x < -this.firstGround.width){
                         if(this.MODE == this.SIMPLE_MODE)   this.firstGround.restart(this.game, this.lastGround.world.x 
                                                                                        + this.distanceVect[this.firstGround.index],this.heightVect[this.firstGround.index]);  
                         else if (this.MODE == this.RANDOM_MODE)  this.firstGround.restart(this.game, this.lastGround.world.x +  this.randomBetween(this.BARRIER_DISTANCE-100 ,this.BARRIER_DISTANCE+50),
           ...
           //PLAYER LOOP
            this.playerGroup.forEachAlive(function(player){
                ...
                if(player.isBest) this.textGroup.getAt(0).text = 'Current Best Player Index: ' + player.index
